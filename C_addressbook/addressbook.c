...
struct address_list *empty(struct address_list *head); // 声明释放内存函数  
...
struct address_list* delete_txt(struct address_list *head)
{
...
//排序函数
struct address_list *Sort(struct address_list *head)
{
	struct address_list *p1, *p2;
	int i,j;
...
struct address_list *search(struct address_list *head)
{
	...
	while(p1 != NULL)
	{
		...
		if(strcmp(p1->name, name) == 0)
		{
			...
			printf("你查找的内容是:\n");
			printf("+++++++++++++++++++++++++++++++++++\n");
			printf("++ %s        %s       %s       %s        %s\n", p1->name, p1->work, p1->phone, p1->email, p1->address);
			printf("+++++++++++++++++++++++++++++++++++\n");
		}
		p1 = p1->next;
		if(m == 0)
		{
			printf("你要查找的此人未在本通讯录中!\n");
		}
		break;
	}
	return(head);
...
//释放内存函数
struct address_list *empty(struct address_list *head)
{
...
struct address_list *load(struct address_list *head)
{
	...
	p1 = (struct address_list *)malloc(LEN);
	fscanf(fp, "%s%s%s%s%s", &p1->name, &p1->work, &p1->phone, &p1->email, &p1->address);
	if (feof(fp) != 0)
	{
		printf("文件为空,无法打开!\n");
		return(head);
